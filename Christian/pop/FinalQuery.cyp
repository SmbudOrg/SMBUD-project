...
MATCH (u:University{name:"Polytechnic University of Milan, Italy"})<-[:AFFILIATED_TO]-(w:Website)-[:HOMEPAGE_OF]->(a:Person)-[:AUTHOR_OF]->(p:Publication)-[:ABOUT]-(k:Keyword {keyword: "data mining"})
...
10. Return the keywords that have the maximum number of citations across phdthesis published by university x
...
ORDER BY avgcit
RETURN j, avgcit LIMIT 50

13. Return the author with the maximum number of citations across publications about data mining and their website
...
14. Return the books published after 2019 that have the highest percentage of incollection elements about machine learning

MATCH (y:Year)<-[:PUBLISHED_IN]-(b:Book)<-[:COLLECTED_IN]-(i:Incollection)-[:ABOUT]-(k:Keyword {keyword: "machine learning"})
WHERE y.year >= 2019
WITH b, COUNT (i) AS icount
ORDER BY icount DESC
RETURN b, icount LIMIT 20 

15. Order publishers by the average number of citations of their works that were published after 2015
...
ORDER BY avgcit DESC
RETURN p, avgcit LIMIT 20

16. Return publications about databases written after 2015 ordering them by the number of pages

MATCH (k:Keyword {keyword:"neo4j"})<-[:ABOUT]-(p:Publication)-[:PUBLISHED_IN]->(y:Year)
WHERE y.year >=2010
WITH p
ORDER BY p.pages ASC
RETURN p, p.pages LIMIT 50

17. Return people that organised (editor of) the most number of conferences (proceedings) held after 2010
...
18. Return authors affiliated with Politecnico di Milano ordering them by the average number of citations across their publications
...
19. Return the 5 proceedings containing the biggest number of inproceedings for each of the last 10 years
...
20. Return website of an editor who has edited at least one book and one proceeding published by the same Publisher (IEEE for example)
...
21. Return the top 10 universities which have produced the higher number of phd thesis regarding the topic of language processing in the last two years
(il dataset include le pubblicazioni fino al 2021 o 2022?)

MATCH (u:University)<-[:PRODUCED BY]-(p:Phedthesis)-[r:ABOUT]->(k:Keyword{keyword:"language processing"})
MATCH (p)-[:PUBLISHED IN]->(y:Year)
WHERE y.year>2020
